...
var Limit = 20; // current search results limit forced by CK
var Offset = 0;
...
function FiltersUpdate() {
	...
	let Generation = [];
	let From = parseInt(document.getElementById("NumberGenerationFrom").value,10);
	let To = parseInt(document.getElementById("NumberGenerationTo").value,10);
	let Type = document.getElementById("SelectType").value;
	Cooldown = Cooldown ? `cooldown:${Cooldown.join(",")}` : "";
	if (From==-1) {
		Generation = "";
	} else {
		do { // pushing at least starting generation
			Generation.push(From++);
		} while (From<=To);
		Generation = `gen:${Generation.join(",")}`; // easiest (& laziest) way to build a string
	};
	Type = Type ? `type:${Type}` : "";
...
function CountKitties() { // getting overall population & cattributes rarity
	ButtonCattributes.disabled = true;
	fetch(LinkAPI+"kitties?limit=1").then((Data)=>(Data.json())).then((Data)=>{ // requesting minimal possible data
		let Population = Data.total;
		ButtonCattributes.removeEventListener("click",CountKitties); // changing handler after initial info is loaded
		...
		fetch(LinkAPI+"cattributes?total=true").then((Data)=>(Data.json())).then((Data)=>{
			Data.forEach((Cattribute)=>{
				Rarities[Cattribute.description] = {
					Total: Cattribute.total,
					Percent: Cattribute.total / Population * 100
				};
			});
			GatherKitties();
		});
	}).catch((Message)=>{console.error("Count error",Message);});
};
...
function LoadKitties(Kitties,Length) { // loading cattributes
	...
	fetch(LinkAPI+"kitties/"+Id).then((Data)=>(Data.json())).then((Data)=>{
		let Cattributes = Data.cattributes.map((Cattribute)=>{
			let Span = document.createElement("Span");
			let Rarity = Rarities[Cattribute.description];
			Rarity = Rarity ? Rarity.Percent : 0.001; // rarity percentage; high rarity assumed if it's not in the list; ToFix
	...
	}).catch((Message)=>{console.error("Load error",Message);});
};
